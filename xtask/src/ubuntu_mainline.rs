...
fn html_tag_href(tag: &str) -> Option<&str> {
    // `html.split('<')` leaves chunks that start with a tag name. First
    // identify anchor tags by the full `a` tag name; `href` is checked below
    // as an attribute because it does not have to be the first attribute.
    ...
    // Apache directory listings use quoted attributes. Walk the attributes
    // until the `href` attribute is found, then return its value. Malformed
    // tags simply do not yield a link.
    loop {
...
fn directory_listing_urls<'a>(
    ...
    html.split('<').filter_map(html_tag_href).map(move |href| {
...
fn ubuntu_mainline_kernel_urls(
    ...
    // Ubuntu's root listing mixes stable releases with other entries, for example
    // `v6.18.48/`, `v7.2/`, `v7.2.2/`, `v7.3-rc1/`, and `daily/`. A selector such
    // as `6.18` considers only `v6.18.*`; `latest-stable` considers every directory
    // name matching `vMAJOR.MINOR[.PATCH]`.
    let mut mainline_versions = directory_listing_urls(&mainline_html, UBUNTU_MAINLINE_URL)
        .filter_map(|url| {
            // Ubuntu exposes an HTML directory listing rather than a stable API,
            // so ignore links that do not match the expected release-name format.
            let name = url_file_name(url.as_ref()).ok()?;
            let mut numbers = name.strip_prefix('v')?.split('.');
            let major = numbers.next()?.parse().ok()?;
            let minor = numbers.next()?.parse().ok()?;

            // Ubuntu names a base release directory `v7.2` but uses `7.2.0` in
            // its package names, so normalize an omitted patch number to zero.
    ...
    // Ubuntu may publish a release directory before the requested architecture's
    // artifacts are available. Limit the search to the newest matching builds so
    // the selector fails rather than silently falling back to an arbitrarily old one.
        ...
        .take(MAX_MAINLINE_BUILD_CANDIDATES)
    {
        ...
        if !directory_listing_urls(&release_html, &release_url).any(|url| url.as_ref() == base_url)
        {
            skipped.push(format!("{mainline_version} missing {architecture}/"));
            continue;
        }

        let packages_html = client
            .get_text(&base_url)
        ...
        for url in directory_listing_urls(&packages_html, &base_url) {
         ...
         entry_type| {
            ...
            if let Some(name) = first_component_after(path, "./boot/") {
                ...
                if name.starts_with(b"vmlinuz-") {
                    return Disposition::Unpack(Some(kernel_images));
                }
            }
         ...
         entry_type| {
            for modules_dir in ["./lib/modules/", "./usr/lib/modules/"] {
                if let Ok(path) = path.strip_prefix(modules_dir) {
                    return Disposition::Unpack((path.iter().count() == 1).then_some(modules_dirs));
                }
            }
            ...
            if let Some(name) = first_component_after(path, "./boot/") {
                ...
                if name.starts_with(b"config-") {
                    return Disposition::Unpack(Some(configs));
                } else if name.starts_with(b"System.map-") {
                    return Disposition::Unpack(Some(system_maps));
                }
            }
