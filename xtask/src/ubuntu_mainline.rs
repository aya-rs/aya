...
fn html_links(html: &str, base_url: &str) -> Vec<String> {
    html.split('<')
        ...
        .map(|href| join_url(base_url, href))
        .collect()
}
...
fn ubuntu_mainline_kernel_urls(
    ...
    let mut mainline_versions = directory_listing_urls(client, UBUNTU_MAINLINE_URL)
        ...
        .take(MAX_MAINLINE_BUILD_CANDIDATES)
    {
        ...
        let release_urls = directory_listing_urls(client, &release_url)
        ...
        if !release_urls.iter().any(|url| url == &base_url) {
        ...
        let Some(image) = find_mainline_package_url(&urls, &image_prefix, architecture)
            .with_context(|| {
                format!("failed to inspect Ubuntu Mainline image package in {base_url}")
            })?
        else {
            skipped.push(format!("{mainline_version} missing generic image package"));
            continue;
        };
        let Some(modules) = find_mainline_package_url(&urls, &modules_prefix, architecture)
            .with_context(|| {
                format!("failed to inspect Ubuntu Mainline modules package in {base_url}")
            })?
        else {
            skipped.push(format!(
    ...
    ) -> Disposition<Option<&'state mut Vec<PathBuf>>>,
{
    ...
    while let Some(entry) = archive_reader.next_entry() {
        ...
        let (data_archive, entry_reader): (&str, Box<dyn io::Read>) =
            match entry.header().identifier() {
                b"data.tar" => ("data.tar", Box::new(entry)),
                b"data.tar.xz" => ("data.tar.xz", Box::new(xz2::read::XzDecoder::new(entry))),
                b"data.tar.zst" => (
                    "data.tar.zst",
                    Box::new(
