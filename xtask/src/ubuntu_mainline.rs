...
fn html_links(html: &str, base_url: &str) -> Vec<String> {
    ...
    while let Some(index) = rest.find("href") {
        ...
        let href = &after_equals[value_start..value_start + value_end];
        links.push(join_url(base_url, href));
        rest = &after_equals[value_start + value_end + quote.len_utf8()..];
    }
    links
}

// Fetch a directory listing page and return its href targets as absolute URLs.
// The directory listings only need quoted hrefs, so avoid adding a full HTML
// parser dependency to xtask for this narrow path.
fn directory_listing_urls(client: &HttpClient, url: &str) -> Result<Vec<String>> {
    let html = client.get_text(url)?;
    Ok(html_links(&html, url))
}

fn mainline_package_url(
    urls: &[String],
    package_prefix: &str,
    architecture: &str,
) -> Result<String> {
    ...
    let matches = urls
        .iter()
        .filter(|url| {
            let Ok(file_name) = url_file_name(url) else {
                return false;
            };
            let Some((package_name, _)) = file_name.split_once('_') else {
                return false;
            };
            // Ubuntu Mainline documents generic and lowlatency kernel
            // packages. The VM runner uses generic packages, and the unsigned
            // image package must be paired with the matching generic modules
            // package.
            // https://wiki.ubuntu.com/Kernel/MainlineBuilds
            package_name.starts_with(package_prefix)
                && package_name.ends_with("-generic")
    ...
    match matches.as_slice() {
        [url] => Ok((*url).clone()),
        [] => bail!("failed to find Ubuntu Mainline package matching {package_prefix}"),
        urls => bail!("ambiguous Ubuntu Mainline packages matching {package_prefix}: {urls:?}"),
    }
}
...
fn ubuntu_mainline_kernel_urls(
    ...
    // For a line such as 6.6, select the newest v6.6.y directory. If that
    // build is missing packages for the requested architecture, fail rather
    // than silently using an older patch release.
    let mainline_version = directory_listing_urls(client, UBUNTU_MAINLINE_URL)
        .context("failed to list Ubuntu Mainline releases")?
        .into_iter()
        .filter_map(|url| {
            let name = url_file_name(&url).ok()?;
        ...
        .map(|(_, name)| name)
        .ok_or_else(|| anyhow!("failed to find Ubuntu Mainline release for {version}"))?;

    let architecture = architecture.as_str();
    // Ubuntu stores each release under an architecture directory, for example
    // https://kernel.ubuntu.com/mainline/v6.6.100/amd64/.
    let base_url = format!("{UBUNTU_MAINLINE_URL}{mainline_version}/{architecture}/");
    let urls = directory_listing_urls(client, &base_url)
        .with_context(|| format!("failed to list Ubuntu Mainline packages at {base_url}"))?;

    // The VM needs the unsigned image to boot and the matching modules package
    // for config, System.map, and the module tree. Keep both on the generic
    // flavor for the same release.
    ...
    let image = mainline_package_url(&urls, &image_prefix, architecture)
        .with_context(|| format!("failed to find Ubuntu Mainline image package in {base_url}"))?;
    let modules = mainline_package_url(&urls, &modules_prefix, architecture)
        .with_context(|| format!("failed to find Ubuntu Mainline modules package in {base_url}"))?;
...
fn download_ubuntu_mainline_kernel_archives(
    ...
    for version in versions {
        ...
        let mut keep_archive = |archive: &Path| -> Result<()> {
            ...
            keep.insert(archive.with_file_name(format!("{}.etag", file_name.to_string_lossy())));
    ...
    ) -> Disposition<Option<&'state mut Vec<PathBuf>>>,
{
    ...
    while let Some(entry) = archive_reader.next_entry() {
        ...
        let data_archive = entry.header().identifier().to_vec();
        let entry_reader: Box<dyn io::Read> = match data_archive.as_slice() {
            b"data.tar" => Box::new(entry),
            b"data.tar.xz" => Box::new(xz2::read::XzDecoder::new(entry)),
            b"data.tar.zst" => Box::new(
        ...
        data_tar_entries += 1;
        let data_archive = String::from_utf8_lossy(&data_archive);
