...
const MIRROR_URL: &str = "https://deb.debian.org/debian/pool/main/l/linux/";
...
pub(crate) fn run(opts: Options) -> Result<()> {
    ...
    for version in &versions {
        let image_regex = format!(
            r"linux-image-{}\.[0-9]+(-[0-9]+)?(\+bpo|\+deb[0-9]+)?-cloud-{}-unsigned_.*\.deb",
        ...
        let debug_regex = format!(
            r"{}-dbg(sym)?_{}",
            regex::escape(base_prefix),
            regex::escape(kernel_suffix)
        );
