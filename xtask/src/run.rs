...
impl CpioEntry {
    /// Create a directory entry. The caller must provide the trailing `/` in the path.
    ...
    /// Create a file entry that streams its content from `source` on write.
    fn file(path: PathBuf, source: PathBuf, mode: u32) -> Result<Self> {
        let filesize = fs::metadata(&source)
    ...
    /// Write the entry to the given writer, streaming file content from disk.
    fn write<W: Write>(&self, writer: &mut W) -> Result<()> {
        ...
        if let Some(source) = &self.source {
            ...
            write_padding(writer, copied as usize)?;
        }
        Ok(())
    }
}
        ...
        Environment::VM {
                ...
                package_groups.into_iter().enumerate()
            {
                ...
                let mut entries = vec![
                    ...
                    CpioEntry::dir(PathBuf::from("/lib/modules/"), 0o755),
                ];
