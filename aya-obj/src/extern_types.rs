...
impl Object {
    ...
    pub fn resolve_typed_externs(&mut self, kernel_btf: &Btf) -> Result<(), KsymsError> {
    ...
    #[cfg(feature = "std")]
    pub fn resolve_typeless_externs(&mut self) -> Result<(), KsymsError> {
        use std::{fs::File, io::BufRead as _};
        let unresolved: Vec<String> = self.unresolved_ksym_vars();
    ...
    /// Returns unresolved ksym variables from [`ExternCollection`].
    #[cfg(feature = "std")]
    fn unresolved_ksym_vars(&self) -> Vec<String> {
...
pub(crate) struct ExternDesc {
    ...
    #[cfg_attr(
        not(feature = "std"),
        expect(dead_code, reason = "only used with std feature")
    )]
