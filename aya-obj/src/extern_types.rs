...
impl Object {
    ...
    pub fn resolve_externs(&mut self, kernel_btf: Option<&Btf>) -> Result<(), KsymsError> {
        ...
        #[cfg(not(feature = "std"))]
        if let Some(name) = self.unresolved_strong_typeless_ksym() {
            return Err(KsymsError::TypelessKsymRequiresResolver { name });
        }
    ...
    /// Returns the first unresolved strong typeless ksym, if any.
    #[cfg(not(feature = "std"))]
    fn unresolved_strong_typeless_ksym(&self) -> Option<String> {
        let Some(obj_btf) = self.btf.as_ref() else {
            return None;
        };

        obj_btf.externs.externs.iter().find_map(|(name, ext)| {
        ...
        S: AsRef<str>,
    {
        ...
        for line in reader.lines() {
            ...
            if !unresolved.iter().any(|s| s.as_ref() == sym_name) {
                continue;
            }
...
pub enum KsymsError {
    ...
    /// A typeless strong ksym requires an address resolver.
    #[error("typeless ksym '{name}' requires an address resolver")]
    TypelessKsymRequiresResolver {
        /// The unresolved typeless strong ksym.
        name: String,
    },
...
mod tests {
    ...
    use alloc::{collections::BTreeMap, ffi::CString, vec, vec::Vec};
    ...
    #[cfg(not(feature = "std"))]
    #[test]
    fn resolve_externs_requires_resolver_for_strong_typeless_ksyms() {
        let mut externs = ExternCollection::new();
        ...
        assert!(matches!(
            err,
            KsymsError::TypelessKsymRequiresResolver { name } if name == "init_task"
        ));
    }
    ...
    mod std_tests {
        ...
        fn resolve_typeless_externs_sets_weak_ksym_addr_to_zero() {
            ...
            object.resolve_typeless_externs().unwrap();
