...
fn parse_btf_map_def(btf: &Btf, info: &DataSecEntry) -> Result<(String, BtfMapDef), BtfError> {
    ...
    //   HashMap -> __0 -> value
    if s.members.len() == 1 {
        let wrapper_field = s.members.first().expect("wrapper should have an element");
        if btf.string_at(wrapper_field.name_offset)?.as_ref() == "__0" {
            ...
            let unsafe_cell_field = unsafe_cell
                .members
                .first()
                .ok_or(BtfError::BtfMapWrapperMissingUnsafeCell)?;
            match btf.string_at(unsafe_cell_field.name_offset)?.as_ref() {
                "value" => {
                    let map_def_type = btf.resolve_type(unsafe_cell_field.btf_type)?;
                    let BtfType::Struct(map_def) = btf.type_by_id(unsafe_cell_field.btf_type)?
                    else {
                        return Err(BtfError::UnexpectedBtfType {
                ...
                _ => return Err(BtfError::BtfMapWrapperMissingUnsafeCell),
            }
        }
        // If the name of the single field is not `__0`, there is a chance that
        // the struct is a very sparse map definition coming from a program
        // written in C. Proceed with it without doing anything additional.
    }
