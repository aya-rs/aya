...
pub enum ParseError {
    ...
    /// Pinning is not supported for legacy (non-BTF) maps
...
fn parse_map_def(name: &str, data: &[u8]) -> Result<bpf_map_def, ParseError> {
    ...
    // tc's pinning modes are not supported.
    // https://github.com/iproute2/iproute2/blob/v6.10.0/include/bpf_elf.h#L27-L29
    if let Some(pinning) = map_def.pinning {
        return Err(ParseError::UnsupportedLegacyPinning {
            name: name.to_owned(),
            pinning: pinning.get(),
        });
    }
    if map_def.inner_id.is_some() || map_def.inner_idx.is_some() {
    ...
    if info.btf_value_type_id != 0 {
        ...
        Map::Legacy(LegacyMap {
            def: bpf_map_def {
                ...
                max_entries: info.max_entries,
                map_flags: info.map_flags,
                pinning: match pinned {
                    PinningType::None => None,
                    PinningType::ByName => NonZeroU32::new(1),
                },
                id: info.id,
                inner_id: None,
                inner_idx: None,
