...
impl Btf {
    ...
    pub(crate) fn fixup_ksyms_datasec(
        ...
        // Extract placeholder variable's name offset and type ID for patching func_proto names and datasec entries.
        // If the placeholder exists: use its name_offset (for string table patching) and btf_type (underlying int type).
        // If no placeholder exists (fallback): search for a 4-byte int type directly, with no name_offset.
