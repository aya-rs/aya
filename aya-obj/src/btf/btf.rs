...
impl<F> BtfFeatureResolver<F> {
    fn new(feature_supported: F) -> Self {
        Self {
            feature_supported,
            cache: HashMap::new(),
        }
    ...
    fn supported(&mut self, feature: BtfFeature) -> bool
    where
        F: FnMut(BtfFeature) -> bool,
    {
        ...
        if let Some(&supported) = cache.get(&feature) {
            return supported;
        }

        let supported = feature_supported(feature);
        cache.insert(feature, supported);
        supported
    }
}
        ...
        F: FnMut(BtfFeature) -> bool,
    {
        ...
        for i in 0..types.types.len() {
            ...
            match t {
                ...
                BtfType::Var(v) if !features.supported(BtfFeature::DataSec) => {
                    *t = BtfType::Int(Int::new(v.name_offset, 1, IntEncoding::None, 0));
                }
                // Sanitize DATASEC if they are not supported.
                BtfType::DataSec(d) if !features.supported(BtfFeature::DataSec) => {
                ...
                BtfType::DataSec(d) if features.supported(BtfFeature::DataSec) => {
                    ...
                    if d.size > 0 {
                        ...
                        if entries.is_empty() && !features.supported(BtfFeature::DataSecZero) {
                ...
                // Fixup FUNC_PROTO.
                BtfType::FuncProto(ty) if features.supported(BtfFeature::Func) => {
                ...
                BtfType::Func(ty) => {
                        ...
                        || name == "memcmp"
                    {
                        ...
                        if ty.linkage() == FuncLinkage::Global {
                            if features.supported(BtfFeature::FuncGlobal) {
...
impl Object {
    ...
    /// `feature_supported` is called only when the BTF data requires a feature, and at most once
...
mod tests {
    ...
    fn fixup_and_sanitize_does_not_probe_unused_features() {
        ...
        btf.fixup_and_sanitize(&HashMap::new(), &HashMap::new(), |_| {
            calls += 1;
            true
        })
        .unwrap();
