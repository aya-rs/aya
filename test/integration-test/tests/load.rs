...
fn unload_kprobe() {
    let bytes = include_bytes_aligned!("../../../target/bpfel-unknown-none/release/test");
    let mut bpf = Bpf::load(bytes).unwrap();
    let prog: &mut KProbe = bpf.program_mut("test_kpr").unwrap().try_into().unwrap();
    prog.load().unwrap();
    assert_loaded!("test_kpr", true);
    let link = prog.attach("try_to_wake_up", 0).unwrap();
    {
        let _link_owned = prog.take_link(link).unwrap();
        prog.unload().unwrap();
        assert_loaded_and_linked!("test_kpr", true);
    };

    assert_loaded!("test_kpr", false);
    prog.load().unwrap();
...
fn basic_tracepoint() {
    let bytes = include_bytes_aligned!("../../../target/bpfel-unknown-none/release/test");
    let mut bpf = Bpf::load(bytes).unwrap();
    let prog: &mut TracePoint = bpf.program_mut("test_tr").unwrap().try_into().unwrap();

    prog.load().unwrap();
    assert_loaded!("test_tr", true);
    let link = prog.attach("syscalls", "sys_enter_kill").unwrap();

    {
        let _link_owned = prog.take_link(link).unwrap();
        prog.unload().unwrap();
        assert_loaded_and_linked!("test_tr", true);
    };

    assert_loaded!("test_tr", false);
    prog.load().unwrap();
...
fn basic_uprobe() {
    let bytes = include_bytes_aligned!("../../../target/bpfel-unknown-none/release/test");
    let mut bpf = Bpf::load(bytes).unwrap();
    let prog: &mut UProbe = bpf.program_mut("test_upr").unwrap().try_into().unwrap();

    prog.load().unwrap();
    assert_loaded!("test_upr", true);
    let link = prog.attach(Some("sleep"), 0, "libc", None).unwrap();

    {
        let _link_owned = prog.take_link(link).unwrap();
        prog.unload().unwrap();
        assert_loaded_and_linked!("test_upr", true);
    };

    assert_loaded!("test_upr", false);
    prog.load().unwrap();
