...
#[test_log::test]
fn test_uprobe_single_program_rejects_multiple_points() {
    if !aya::features().bpf_cookie() {
        eprintln!(
            "skipping test: bpf_get_attach_cookie is unsupported so the test program cannot load"
        );
    ...
    // Load a legacy single-attach program and ensure the unified attach API works.
    let mut bpf = EbpfLoader::new()
    ...
    let attach_res = prog.attach(points, Path::new("/proc/self/exe"), None);
    match attach_res {
        Err(ProgramError::UProbeError(UProbeError::SingleAttachPointRequired { count })) => {
            assert_eq!(count, 2);
        }
        Err(err) => panic!("unexpected error for multiple single-mode points: {err:?}"),
        Ok(link) => panic!("attach unexpectedly succeeded in single mode: {link:?}"),
    }
}
