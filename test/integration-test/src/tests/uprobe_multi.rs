...
use aya::{
    ...
    programs::{
        ProgramError, UProbe,
        uprobe::{UProbeAttachLocation, UProbeAttachPoint, UProbeError, UProbeLinkId},
    },
    util::KernelVersion,
};
...
fn test_uprobe_attach_multi() {
    ...
    let attach_res = match prog {
        UProbe::Multi(p) => p.attach(points, Path::new("/proc/self/exe"), None),
        UProbe::Single(_) => panic!("expected multi-capable program"),
...
#[test_log::test]
fn test_uprobe_single_program_uses_single_handle() {
    if !aya::features().bpf_cookie() {
        eprintln!(
            "skipping test: bpf_get_attach_cookie is unsupported so the test program cannot load"
        );
    ...
    // Load a legacy single-attach program; the loader should expose it as `UProbe::Single`.
    let mut bpf = EbpfLoader::new()
    ...
    let point = UProbeAttachPoint {
        location: UProbeAttachLocation::Symbol("uprobe_cookie_trigger_ebpf_program_a"),
        cookie: None,
    };

    match prog {
        UProbe::Single(p) => {
            let _link_id = p.attach(point, Path::new("/proc/self/exe"), None).unwrap();
        }
...
fn test_uprobe_attach_multi_invalid_symbol() {
    ...
    match attach_res {
        ...
        Ok(link) => {
            ...
            panic!("attach succeeded for invalid symbol");
        }
    }
}
