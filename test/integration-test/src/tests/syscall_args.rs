...
fn syscall_arg_splice() {
    ...
    results
        .set(RESULT_INDEX, &TestResult::default(), 0)
        .unwrap();
    ...
    let TestResult {
        ...
        len,
        flags,
    } = results.get(&RESULT_INDEX, 0).unwrap();
    assert_eq!(ran, RAN, "kprobe did not fire");
    assert_eq!(fd_in, SPLICE_FD_IN, "syscall arg 0 (fd_in)");
    assert_eq!(off_in, SPLICE_OFF_IN, "syscall arg 1 (off_in)");
    assert_eq!(fd_out, SPLICE_FD_OUT, "syscall arg 2 (fd_out)");
    assert_eq!(off_out, SPLICE_OFF_OUT, "syscall arg 3 (off_out)");
    assert_eq!(len, SPLICE_LEN, "syscall arg 4 (len)");
    assert_eq!(flags, SPLICE_FLAGS, "syscall arg 5 (flags)");
}
