...
#[unsafe(no_mangle)]
#[inline(never)]
extern "C" fn trigger_bloom_insert(result_index: u64, value: u64) {
...
#[unsafe(no_mangle)]
#[inline(never)]
extern "C" fn trigger_bloom_contains(result_index: u64, value: u64) -> u64 {
    core::hint::black_box(result_index);
    core::hint::black_box(value);
    result_index + value
}
...
fn bloom_filter_basic() {
    ...
    const PRESENT: u64 = 0xdead_beef;
    const ABSENT: u64 = 0xface_cafe;

    trigger_bloom_insert(INSERT_INDEX as u64, PRESENT);
    assert_eq!(array.get(&INSERT_INDEX, 0).unwrap(), 0);

    trigger_bloom_contains(CONTAINS_PRESENT_INDEX as u64, PRESENT);
    assert_eq!(array.get(&CONTAINS_PRESENT_INDEX, 0).unwrap(), 0);

    trigger_bloom_contains(CONTAINS_ABSENT_INDEX as u64, ABSENT);
    assert_eq!(
