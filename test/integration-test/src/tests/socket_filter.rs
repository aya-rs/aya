...
use aya::{
    ...
    maps::{Array, MapData},
    programs::{
        ProgramError, ProgramType, SocketFilter, SocketFilterAttachType, SocketFilterError,
    },
...
use libc::{EINVAL, ENOENT, SOMAXCONN};
use tokio::{
...
async fn accept_from_either(first: &TcpListener, second: &TcpListener) -> usize {
    ...
    let first_accept = async {
        let (_stream, _) = first
            .accept()
            .await
            .expect("failed to accept on first listener");
        REUSEPORT_FIRST_LISTENER_INDEX as usize
    };
    let second_accept = async {
        let (_stream, _) = second
            .accept()
            .await
            .expect("failed to accept on second listener");
        REUSEPORT_SECOND_LISTENER_INDEX as usize
    };

    let accepted = select(Box::pin(first_accept), Box::pin(second_accept));
    match timeout(ACCEPT_TIMEOUT, accepted)
        .await
        .expect("timed out waiting for accept")
    {
        Either::Left((index, _)) | Either::Right((index, _)) => index,
    }
}
...
async fn socket_filter_program_can_pass_packets() {
    ...
    let prog: &mut SocketFilter = ebpf
        ...
        .unwrap();
    prog.load().unwrap();
    prog.attach(&receiver, SocketFilterAttachType::SocketFilter)
        .unwrap();
...
async fn socket_filter_program_can_trim_packets_and_detach() {
    ...
    SocketFilter::detach(&receiver, SocketFilterAttachType::SocketFilter).unwrap();
...
async fn socket_filter_attach_types_use_separate_slots() {
    ...
    if skip_if_reuseport_detach_unsupported() {
        return;
    }
...

    {
        let prog: &mut SocketFilter = ebpf
            ...
            .unwrap();
        prog.load().unwrap();
        prog.attach(&listener, SocketFilterAttachType::SocketFilter)
            .unwrap();
    }

    // Attaching a regular socket filter must not populate the reuseport
    // selector slot.
    let err = SocketFilter::detach(&listener, SocketFilterAttachType::ReusePort).unwrap_err();
    assert_matches!(
        err,
    ...
    SocketFilter::detach(&listener, SocketFilterAttachType::SocketFilter)
        .expect("regular socket filter should still be attached");

    {
        let prog: &mut SocketFilter = ebpf
            ...
            .unwrap();
        prog.load().unwrap();
        prog.attach(&listener, SocketFilterAttachType::ReusePort)
            .unwrap();
    }
    ...
    let err = SocketFilter::detach(&listener, SocketFilterAttachType::SocketFilter).unwrap_err();
    assert_matches!(
        err,
        ProgramError::SocketFilterError(SocketFilterError::SetsockoptError {
    ...
    // type above.
    SocketFilter::detach(&listener, SocketFilterAttachType::ReusePort)
    ...
    SocketFilter::detach(&listener, SocketFilterAttachType::ReusePort)
        .expect("failed to detach reuseport selector");
    SocketFilter::detach(&listener, SocketFilterAttachType::SocketFilter)
        .expect("failed to detach regular socket filter");
}
...
async fn socket_filter_reuseport_selects_listener_and_detaches() {
    ...
    let prog: &mut SocketFilter = ebpf
        ...
        .unwrap();
    prog.load().unwrap();
    prog.attach(&first, SocketFilterAttachType::ReusePort)
        .unwrap();
    ...
    SocketFilter::detach(&second, SocketFilterAttachType::ReusePort).unwrap();

    let err = SocketFilter::detach(&first, SocketFilterAttachType::ReusePort).unwrap_err();
    assert_matches!(
        err,
        ProgramError::SocketFilterError(SocketFilterError::SetsockoptError {
            option: "SO_DETACH_REUSEPORT_BPF",
...
async fn socket_filter_reuseport_stays_attached_after_ebpf_drop() {
    ...
    SocketFilter::detach(&first, SocketFilterAttachType::ReusePort).unwrap();
...
async fn socket_filter_reuseport_replacement_uses_latest_program() {
    ...
    // must not detach it.
    {
        let first_prog: &mut SocketFilter = ebpf
            .program_mut("select_first")
            .unwrap()
            .try_into()
            .unwrap();
        ...
        first_prog
            .attach(&first, SocketFilterAttachType::ReusePort)
            .unwrap();
    }
    ...
    {
        let second_prog: &mut SocketFilter = ebpf
            .program_mut("select_second")
            .unwrap()
            .try_into()
            .unwrap();
...
async fn socket_filter_reuseport_errors_without_reuseport() {
    ...
    let err = prog
        .attach(&listener, SocketFilterAttachType::ReusePort)
        .unwrap_err();
    assert_matches!(
        err,
        ProgramError::SocketFilterError(SocketFilterError::SetsockoptError {
...
async fn socket_filter_replacement_stays_attached_until_explicit_detach() {
    ...
    pass_prog
        .attach(&receiver, SocketFilterAttachType::SocketFilter)
        .unwrap();
