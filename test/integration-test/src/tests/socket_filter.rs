use std::{os::unix::net::UnixDatagram as StdUnixDatagram, time::Duration};
...
#[test_log::test(tokio::test)]
async fn socket_filter_duplicate_attach_through_cloned_fd_is_rejected() {
    if !is_program_supported(ProgramType::SocketFilter).unwrap() {
        eprintln!("skipping test - socket_filter program not supported");
        return;
    }

    let (_sender, receiver) = StdUnixDatagram::pair().unwrap();
    let receiver_clone = receiver.try_clone().unwrap();
    ...
    assert_matches!(
        prog.attach(&receiver_clone),
        Err(ProgramError::AlreadyAttached)
    );
}

#[test_log::test(tokio::test)]
async fn socket_filter_owned_link_drop_detaches_after_original_fd_closed() {
    if !is_program_supported(ProgramType::SocketFilter).unwrap() {
        eprintln!("skipping test - socket_filter program not supported");
        return;
    }

    // Create std sockets first so the receiver fd can be cloned before
    // converting the sockets used for I/O to tokio.
    let (sender, receiver) = StdUnixDatagram::pair().unwrap();
    let receiver_clone = receiver.try_clone().unwrap();
    sender.set_nonblocking(true).unwrap();
    receiver_clone.set_nonblocking(true).unwrap();
    let sender = UnixDatagram::from_std(sender).unwrap();
    let receiver_clone = UnixDatagram::from_std(receiver_clone).unwrap();
    ...
    send_and_assert(&sender, &receiver_clone, TRIM_DELTA_BYTES as usize).await;
    assert!(
        read_hits(&path_hits, TRIM_HITS_INDEX) > hits_before_attach,
        "trim path did not run",
    );

    // Keep another fd for the same socket so the test can observe whether
    // dropping the link detaches the filter after the original fd is closed.
    // Without the fd owned by the link, detach would use the closed fd number
    // and fail to remove the filter.
    drop(receiver);
    drop(link);

    send_and_assert(&sender, &receiver_clone, 0).await;
}
