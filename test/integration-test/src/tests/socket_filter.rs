...
async fn socket_filter_detach_restores_unfiltered_delivery() {
    ...
    sender
        .send_to(b"pass-packet", receiver.local_addr().unwrap())
        .await
        .unwrap();
    assert_eq!(recv_payload(&receiver).await, b"pass-packet");
    assert!(
    ...
    sender
        .send_to(b"after-detach", receiver.local_addr().unwrap())
        .await
        .unwrap();
    assert_eq!(recv_payload(&receiver).await, b"after-detach");
    assert_eq!(
...
async fn socket_filter_duplicate_attach_keeps_existing_filter() {
    ...
    sender
        .send_to(b"before-duplicate", receiver.local_addr().unwrap())
        .await
        .unwrap();
    assert_eq!(recv_payload(&receiver).await, b"before-duplicate");
    ...
    sender
        .send_to(b"after-duplicate", receiver.local_addr().unwrap())
        .await
        .unwrap();
    assert_eq!(recv_payload(&receiver).await, b"after-duplicate");
    assert!(
...
async fn socket_filter_owned_link_drop_restores_unfiltered_delivery() {
    ...
    sender
        .send_to(b"after-link-drop", receiver.local_addr().unwrap())
        .await
        .unwrap();
    assert_eq!(recv_payload(&receiver).await, b"after-link-drop");
}
