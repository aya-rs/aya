...
fn ring_buf() {
    ...
    assert_eq!(
        Registers::from(regs.get(&0, 0).unwrap()),
        Registers {
...
async fn ring_buf_async_with_drops() {
    ...
    // before epoll has an opportunity to send a notification.
    let sleep_fut = Box::pin(sleep(Duration::from_millis(100)));
    ...
    let Registers { dropped, rejected } = Registers::from(regs.get(&0, 0).unwrap());
