use core::panic;
use std::{os::fd::AsRawFd as _, sync::atomic::Ordering, thread};
...
fn ring_buf_epoll_wakeup() {
    ...
    let threads = WriterThreads::spawn();
    while total_events < WriterThreads::NUM_MESSAGES {
...
// WriterThreads is used by both the epoll and async fd test that
...
impl WriterThreads {
    // This is enough messages for the test to fail relatively reliably in a hardware accelerated VM
    // with 2 vCPUs when the bug was present.
    const NUM_MESSAGES: u64 = 20_000;

    fn spawn() -> Self {
        let done = std::sync::Arc::new(std::sync::atomic::AtomicBool::new(false));
        let threads: Vec<_> = (0..4/* num_threads */)
            .map(|_: usize| {
                let done = done.clone();
                thread::spawn(move || {
                    while !done.load(Ordering::Relaxed) {
                        ring_buf_trigger_ebpf_program(2);
                    }
                })
            })
            .collect();
