...
use aya::{
    maps::{array::PerCpuArray, ring_buf::RingBuf, MapData, PerCpuValues},
...
impl std::iter::Sum<PerCpuValues<Registers>> for Registers {
    fn sum<I: Iterator<Item = PerCpuValues<Registers>>>(iter: I) -> Self {
        iter.fold(Default::default(), |a, b| a + b.into())
    }
}

impl From<PerCpuValues<Registers>> for Registers {
    fn from(regs: PerCpuValues<Self>) -> Self {
        regs.iter().fold(Default::default(), |a, b| a + *b)
    }
}
...
fn ring_buf() {
    ...
    assert_eq!(
        regs.iter().map(Result::unwrap).sum::<Registers>(),
        Registers {
...
async fn ring_buf_async_with_drops() {
    ...
    let Registers { dropped, rejected } = regs.iter().map(Result::unwrap).sum::<Registers>();
