...
fn bpf_printk_variadic_args() {
    ...
    let u32_expected = format!("PRINTK_TEST_U32:{TEST_U32:x}");
    assert!(
        trace_content.contains(&u32_expected),
        "u32 test failed - expected '{u32_expected}'\ntrace:\n{trace_content}"
    );

    let u64_expected = format!("PRINTK_TEST_U64:{TEST_U64:x}");
    assert!(
    ...
    let multi_expected = format!("PRINTK_TEST_MULTI:{TEST_U8},{TEST_U32:x}");
    assert!(
