...
use libc::{
    AF_INET, EINVAL, ENOENT, SHUT_RDWR, SO_REUSEPORT, SOCK_STREAM, SOL_SOCKET, bind, in_addr,
    listen, setsockopt, shutdown, sockaddr, sockaddr_in, socket, socklen_t,
};
...
fn reuseport_listener(port: u16) -> io::Result<TcpListener> {
    ...
    // so the test creates and configures the socket with `libc`.
...
fn sk_reuseport_selects_expected_listener() {
    ...
    assert!(wait_for_hits(&path_hits, SELECT_HITS_INDEX));
...
fn sk_reuseport_clear_index_changes_selection() {
...

    {
        // Limit the mutable borrow of `ebpf` from `program_mut()` to this block.
        let prog: &mut SkReuseport = ebpf
            .program_mut("select_socket_after_clear")
            .unwrap()
            .try_into()
            .unwrap();
        prog.load().unwrap();
    ...
    assert!(wait_for_hits(&path_hits, CLEAR_FALLBACK_HITS_INDEX));
...
fn sk_reuseport_migrates_to_expected_listener() {
    ...
    let prog: &mut SkReuseport = ebpf
        .program_mut("select_or_migrate_socket")
        .unwrap()
        .try_into()
        .unwrap();
    match prog.load() {
        ...
        Err(err) => panic!("unexpected error loading sk_reuseport/migrate program: {err}"),
    }
    ...
    let ret = unsafe { shutdown(listener_a.as_raw_fd(), SHUT_RDWR) };
    assert_eq!(
        ret,
        0,
        "shutdown(listener_a) failed: {}",
