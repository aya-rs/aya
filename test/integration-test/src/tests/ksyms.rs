...
fn ksyms_typed_strong() {
    ...
    if let Err(e) = prog.load("sys_enter", &btf) {
        // Symbol may be in BTF but verifier can't resolve address (per-CPU static)
        if let aya::programs::ProgramError::LoadError { verifier_log, .. } = &e {
            if verifier_log.to_string().contains("failed to find the address") {
                eprintln!("skipping test, verifier cannot resolve bpf_prog_active address");
                return;
            }
        }
