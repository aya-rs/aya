...
fn read_counter() {
    ...
    assert!(disabled_read.enabled > Duration::ZERO);
    assert!(disabled_read.running > Duration::ZERO);
    let disabled_values: [u64; 2] = disabled_read.values.try_into().unwrap();
    let disabled_enabled = disabled_read.enabled;
    let disabled_running = disabled_read.running;
    ...
    assert_eq!(disabled_read.enabled, disabled_enabled);
    assert_eq!(disabled_read.running, disabled_running);
    ...
    assert!(resumed_read.enabled > disabled_enabled);
    assert!(resumed_read.running > disabled_running);
    let resumed_enabled = resumed_read.enabled;
    let resumed_running = resumed_read.running;
    ...
    assert_eq!(reset_read.enabled, resumed_enabled);
    assert_eq!(reset_read.running, resumed_running);
}
