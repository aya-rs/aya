...
use aya::{
    ...
    programs::{
        UProbe,
        perf_event::{
            PerfEventConfig, PerfEventGroup, PerfEventGroupRead, PerfEventScope, SoftwareEvent,
        },
        uprobe::UProbeScope,
    },
    util::online_cpus,
...
fn read_counter() {
    ...
    let disabled = {
        ...
        assert!(enabled_nanos > 0);
        assert!(running_nanos > 0);
        let values: [u64; 2] = values.try_into().unwrap();
        ReadSnapshot {
            values,
            enabled_nanos,
            running_nanos,
    ...
    let [task_clock, _, _, cpu_clock, _, _] = disabled_ebpf_read;
    assert_eq!([task_clock, cpu_clock], disabled.values);
...

    {
        ...
        assert_eq!(values, disabled.values);
        assert_eq!(enabled_nanos, disabled.enabled_nanos);
        assert_eq!(running_nanos, disabled.running_nanos);
    }
    ...
    let resumed = {
        ...
        assert!(enabled_nanos > disabled.enabled_nanos);
        assert!(running_nanos > disabled.running_nanos);
        let values: [u64; 2] = values.try_into().unwrap();
        ReadSnapshot {
            values,
            enabled_nanos,
    ...
    group.reset().unwrap();
    {
        ...
        assert!(values.iter().all(|value| *value == 0));
        assert_eq!(enabled_nanos, resumed.enabled_nanos);
        assert_eq!(running_nanos, resumed.running_nanos);
    }
}
