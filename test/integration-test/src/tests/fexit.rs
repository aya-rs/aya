...
use integration_common::fexit::{
    ARG_MISMATCH, NO_ERROR, RETVAL_MISMATCH, TEST1_INDEX, TEST2_INDEX, TEST3_INDEX, TEST4_INDEX,
    TEST5_INDEX, TEST6_INDEX, TEST7_INDEX, TEST8_INDEX, TEST9_INDEX, TEST10_INDEX, TestResult,
};
...
fn fexit_reads_args_and_return_values_from_prog_test_run_targets(
    ...
    assert!(actual.ran, "{target} was not called");
    assert_eq!(
        actual.error,
        NO_ERROR,
        "{target} failed: {} ({})",
