...
use integration_common::stack_trace::{RESULT_RAN, RESULT_STACKID};
...
fn record_stackid(stacks_map: &str, result_map: &str, prog: &str) {
    ...
    assert_eq!(
        result.get(&RESULT_RAN, 0).unwrap(),
        1,
        "probe {prog} did not run",
    );
    let stack_id = result.get(&RESULT_STACKID, 0).unwrap();
