...
#define TRISTATE_ENUM_INDEX (BOOL_VALUE_INDEX + 1)
#define TRUNCATED_STRING_INDEX (TRISTATE_ENUM_INDEX + 1)
...
int test_kconfig(void *ctx) {
  ...
  if (set_result(UNKNOWN_LINUX_INDEX, LINUX_HAS_FUTURE_FEATURE) != 0) {
    return 1;
  }
  if (set_result(CHAR_VALUE_INDEX, (__s64)(__u8)CONFIG_CHAR_VALUE) != 0) {
    return 1;
  }
  ...
  for (int i = 0; i < TRUNCATED_STRING_LEN; i++) {
    if (set_result(TRUNCATED_STRING_INDEX + i,
                   (__s64)(__u8)CONFIG_TRUNCATED_STRING[i]) != 0) {
      return 1;
    }
  }
