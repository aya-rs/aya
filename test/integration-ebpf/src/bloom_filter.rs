...
#[map]
static RESULT: Array<c_long> = Array::<i64>::with_max_entries(RESULT_SLOTS, 0);

#[map]
static FILTER: BloomFilter<u32> = BloomFilter::with_max_entries(64, 0);

#[inline(always)]
fn map_result(result: Result<(), c_long>) -> c_long {
    match result {
        Ok(()) => 0,
        Err(err) => err,
    }
...
#[uprobe]
fn bloom_filter_insert(ctx: ProbeContext) -> Result<(), c_long> {
...
#[uprobe]
fn bloom_filter_contains(ctx: ProbeContext) -> Result<(), c_long> {
    let index: u32 = ctx.arg(0).ok_or(-1)?;
    let value: u32 = ctx.arg(1).ok_or(-1)?;
