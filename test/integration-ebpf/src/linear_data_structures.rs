...
macro_rules! define_linear_ds_test {
    ($map:ident, $result_map:ident,
        ...
        define_linear_ds_test!(@probe $map, $result_map, $pop_fn, pop, POP_INDEX $(, $unwrap)?);
        define_linear_ds_test!(@probe $map, $result_map, $peek_fn, peek, PEEK_INDEX $(, $unwrap)?);
    };
    ...
    (@probe $map:ident, $result_map:ident, $fn:ident, $method:ident, $idx:ident
        ...
        fn $fn(_: ProbeContext) -> Result<(), c_long> {
            let value = $map.$method()$($unwrap)?.ok_or(-1)?;
