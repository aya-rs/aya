...
macro_rules! define_reuseport_programs {
    ($map:ident, $select:ident, $clear:ident, $migrate:ident $(,)?) => {
        ...
        fn $select(ctx: SkReuseportContext) -> u32 {
            ...
            match $map.select_reuseport(&ctx, SELECT_SOCKET_INDEX, 0) {
                Ok(()) => SK_PASS,
                Err(_) => SK_DROP,
            }
        }
        ...
        fn $clear(ctx: SkReuseportContext) -> u32 {
            ...
            match $map.select_reuseport(&ctx, SELECT_SOCKET_INDEX, 0) {
                ...
                Err(_) => match $map.select_reuseport(&ctx, MIGRATE_SOCKET_INDEX, 0) {
                    Ok(()) => {
        ...
        fn $migrate(ctx: SkReuseportContext) -> u32 {
            ...
            match $map.select_reuseport(&ctx, socket_idx, 0) {
                Ok(()) => SK_PASS,
                Err(_) => SK_DROP,
            }
        }
