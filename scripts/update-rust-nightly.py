...
def main() -> None:
    ...
    if arguments == ["latest"]:
        write_output(latest_nightly())
        return

    if arguments and (len(arguments) != 2 or arguments[0] != "update"):
        raise SystemExit(f"usage: {sys.argv[0]} [latest|update NIGHTLY]")
    ...
    match = re.fullmatch(r"nightly-(\d{4}-\d{2}-\d{2})", arguments[1])
    if match is None:
