...
class ManifestPackages(TypedDict):
    rustc: RustPackage


class NightlyManifest(TypedDict):
    date: str
    pkg: ManifestPackages
...
def latest_nightly() -> tuple[str, NightlyManifest]:
    ...
    rust_commit = rustc.get("git_commit_hash")
    if not isinstance(rust_commit, str) or re.fullmatch(SHA, rust_commit) is None:
        raise ValueError("nightly manifest contains an invalid rustc commit")

    validated: NightlyManifest = {
        "date": date,
        "pkg": {"rustc": {"git_commit_hash": rust_commit}},
    }
    return f"nightly-{datetime.date.fromisoformat(date).isoformat()}", validated
...
def main(
    ...
    update: Callable[[str, Pin, str, NightlyManifest], str | None],
) -> int:
