...
use libc::{syscall, SYS_open, O_DIRECTORY, O_RDWR};
use log::{debug, warn};
use thiserror::Error;
...
pub(crate) fn create_token<P: AsRef<Path>>(path: P) -> Result<OwnedFd, EbpfError> {
    ...
    let bpffs_fd = unsafe { syscall(SYS_open, path_string.as_ptr(), O_DIRECTORY, O_RDWR) };

    if bpffs_fd < 0 {
        return Err(EbpfError::IOError(io::Error::last_os_error()));
    }
    let bpffs_fd = unsafe { OwnedFd::from_raw_fd(bpffs_fd as i32) };
