...
impl<'a> EbpfLoader<'a> {
    ...
    pub fn load(&mut self, data: &[u8]) -> Result<Ebpf, EbpfError> {
        ...
        obj.sanitize_functions(|| FEATURES.bpf_probe_read_kernel.get());
        ...
        let programs = obj
            ...
            .map(|(name, prog_obj)| {
                ...
                let prog_name = FEATURES.bpf_name.get().then(|| name.clone().into());
...
fn value_size_override(map_type: bpf_map_type) -> Option<u32> {
    match map_type {
        bpf_map_type::BPF_MAP_TYPE_CPUMAP => {
            Some(if FEATURES.cpumap_prog_id.get() { 8 } else { 4 })
        }
        bpf_map_type::BPF_MAP_TYPE_DEVMAP | bpf_map_type::BPF_MAP_TYPE_DEVMAP_HASH => {
            Some(if FEATURES.devmap_prog_id.get() { 8 } else { 4 })
        }
