...
impl<'a> EbpfLoader<'a> {
    ...
    /// Sets a factory for lazily loading the target [BTF](Btf) info.
    ///
    ...
    ///
    /// let bpf = EbpfLoader::new()
    ///     .btf_source(|| fs::read("/custom_btf_file"))
    ///     .load_file("file.o")?;
    ///
    /// # Ok::<(), aya::EbpfError>(())
    /// ```
    pub fn btf_source<F, D>(&mut self, source: F) -> &mut Self
    where
        F: Fn() -> io::Result<D>
            + Send
            + Sync
        ...
        D: AsRef<[u8]>,
    {
        self.btf = TargetBtf::Source(BtfSource(Box::new(move |parse| {
            let data = source().map_err(EbpfError::BtfSourceError)?;
            parse(data.as_ref()).map_err(EbpfError::BtfError)
        })));
        self
    }
