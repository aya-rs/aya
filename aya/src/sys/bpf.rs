...
use aya_obj::{
    ...
    generated::{
        BPF_ALU64, BPF_CALL, BPF_DW, BPF_EXIT, BPF_F_REPLACE, BPF_F_TEST_RUN_ON_CPU, BPF_IMM,
        BPF_JMP, BPF_K, BPF_LD, BPF_MEM, BPF_MOV, BPF_PSEUDO_MAP_VALUE, BPF_ST, bpf_attach_type,
        bpf_attr, bpf_attr__bindgen_ty_7, bpf_btf_info, bpf_cmd, bpf_func_id, bpf_insn,
        bpf_link_info, bpf_map_info, bpf_map_type, bpf_prog_info, bpf_prog_type, bpf_stats_type,
    },
...
fn is_probe_read_kernel_supported_inner(token_fd: Option<BorrowedFd<'_>>) -> bool {
    ...
    with_prog_insns(ProgramType::TracePoint, &insns, |attr| {
        if let Some(token_fd) = token_fd {
            let u = unsafe { &mut attr.__bindgen_anon_3 };
            u.prog_token_fd = token_fd.as_raw_fd();
        }
...
fn is_bpf_cookie_supported_inner(token_fd: Option<BorrowedFd<'_>>) -> bool {
    ...
    with_prog_insns(ProgramType::KProbe, &insns, |attr| {
        if let Some(token_fd) = token_fd {
            let u = unsafe { &mut attr.__bindgen_anon_3 };
            u.prog_token_fd = token_fd.as_raw_fd();
        }
