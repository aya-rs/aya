...
use libc::{c_void, AT_FDCWD, ENOENT, ENOSPC};
use obj::{
...
impl FilesystemPermissionsBuilder {
    /// Set the UID of the permissions.
    pub fn uid(&mut self, uid: u32) -> &mut Self {
        self.uid = Some(uid);
        self
    }

    /// Set the GID of the permissions.
...
pub fn create_bpf_filesystem<P: AsRef<Path>>(
    ...
    if let Some(uid) = perms.uid {
        let uid_str = CString::new(uid.to_string()).unwrap();
        let _ = syscall(Syscall::FsConfig {
            fd: fd.as_fd(),
            cmd: FSCONFIG_SET_STRING,
            key: c"uid".as_ptr(),
            value: uid_str.as_ptr() as *const c_void,
            aux: 0,
        })
        .map_err(|(_, io_error)| SyscallError {
            call: "fsconfig",
    ...
    if let Some(gid) = perms.gid {
        let gid_str = CString::new(gid.to_string()).unwrap();
        let _ = syscall(Syscall::FsConfig {
            fd: fd.as_fd(),
            cmd: FSCONFIG_SET_STRING,
            key: c"gid".as_ptr(),
            value: gid_str.as_ptr() as *const c_void,
            aux: 0,
        })
        .map_err(|(_, io_error)| SyscallError {
            call: "fsconfig",
