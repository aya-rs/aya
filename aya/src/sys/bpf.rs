...
/// Options for running a BPF program in test mode.
#[derive(Debug, Default)]
pub struct TestRunOptions<'a> {
    /// Input data to be passed to the program.
    pub data_in: Option<&'a [u8]>,
    /// Buffer to receive output data from the program.
    pub data_out: Option<&'a mut [u8]>,
    /// Input context to be passed to the program.
    pub ctx_in: Option<&'a [u8]>,
    /// Buffer to receive output context from the program.
...
pub struct TestRunResult {
    ...
    /// Size of the output context.
    pub ctx_size_out: u32,
}

/// Runs a BPF program in test mode.
///
...
pub(crate) fn bpf_prog_test_run(
    ...
    if let Some(data_out) = &mut opts.data_out {
    ...
    if let Some(ctx_out) = &mut opts.ctx_out {
