...
pub fn is_map_supported(map_type: MapType) -> Result<bool, SyscallError> {
    ...
    match io_error.raw_os_error() {
        ...
        // `bpf_attr` fields unknown to the kernel triggers `E2BIG` from `bpf_check_uarg_tail_zero()`
        // (https://elixir.bootlin.com/linux/v4.18/source/kernel/bpf/syscall.c#L71).
        Some(E2BIG)
            if matches!(
                map_type,
                MapType::SkStorage
