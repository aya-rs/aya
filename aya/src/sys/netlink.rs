...
use aya_obj::generated::{
    ...
    TC_H_UNSPEC, TCA_BPF_CLASSID, TCA_BPF_FD, TCA_BPF_FLAG_ACT_DIRECT, TCA_BPF_FLAGS, TCA_BPF_NAME,
...
use libc::{
    ...
    RTM_GETTFILTER, RTM_NEWQDISC, RTM_NEWTFILTER, RTM_SETLINK, SOCK_RAW, SOL_NETLINK, UIO_MAXIOV,
    getsockname, iovec, nlattr, nlmsgerr, nlmsghdr, recv, setsockopt, sockaddr_nl, socket, writev,
};
use thiserror::Error;
...
// Keep each input alive until send, evaluating it only once. The accumulated
// slices determine both the wire lengths and the statically sized iovec array.
// A nested attribute resumes its parent after collecting all of its children.
macro_rules! send_netlink {
    ($sock:expr, $kind:expr, $flags:expr, [$($parts:tt)*]) => {{
        let sock = &$sock;
        let kind = $kind;
        let flags = $flags as u16;
        send_netlink!(@parts [] [$($parts)*] [send(sock, kind, flags)])
    }};
    (@parts [$($buf:expr,)*] [] [send($sock:ident, $kind:ident, $flags:ident)]) => {{
        let bufs: [&[u8]; _] = [$($buf,)*];
        let header = nlmsghdr {
    ...
    (@parts [$($buf:expr,)*] [] [nested($kind:ident) [$($parent:expr,)*] [$($rest:tt)*] $resume:tt]) => {{
        let bufs: [&[u8]; _] = [$($buf,)*];
        let header = attr_header($kind | NLA_F_NESTED as u16, bufs.iter().map(|buf| buf.len()).sum());
        send_netlink!(@parts [$($parent,)* bytes_of(&header), $($buf,)*] [$($rest)*] $resume)
    }};
    (@parts $bufs:tt [nested_attr($kind:expr, [$($children:tt)*]), $($rest:tt)*] $resume:tt) => {{
        let kind = $kind;
        send_netlink!(@parts [] [$($children)*] [nested(kind) $bufs [$($rest)*] $resume])
    }};
    (@parts $bufs:tt [value($value:expr), $($rest:tt)*] $resume:tt) => {{
        let value = $value;
        send_netlink!(@parts $bufs [bytes(bytes_of(&value)), $($rest)*] $resume)
    }};
    (@parts [$($buf:expr,)*] [bytes($bytes:expr), $($rest:tt)*] $resume:tt) => {{
        let bytes: &[u8] = $bytes;
        send_netlink!(@parts [$($buf,)* bytes, padding(bytes.len()),] [$($rest)*] $resume)
    }};
    (@parts $bufs:tt [attr($kind:expr, value($value:expr)), $($rest:tt)*] $resume:tt) => {{
        let kind = $kind;
        let value = $value;
        send_netlink!(@parts $bufs [attr(kind, bytes(bytes_of(&value))), $($rest)*] $resume)
    }};
    (@parts [$($buf:expr,)*] [attr($kind:expr, bytes($bytes:expr)), $($rest:tt)*] $resume:tt) => {{
        let kind = $kind;
        let bytes: &[u8] = $bytes;
        let header = attr_header(kind, bytes.len());
        send_netlink!(@parts [$($buf,)* bytes_of(&header), bytes, padding(bytes.len()),] [$($rest)*] $resume)
    }};
    (@parts [$($buf:expr,)*] [optional_attr($kind:expr, $value:expr), $($rest:tt)*] $resume:tt) => {{
        let kind = $kind;
        let value = $value;
        let bytes = value.as_ref().map(bytes_of);
        ...
        [
            ...
            nested_attr(
                ...
                [
                    attr(
                        IFLA_XDP_FD as u16,
                        value(fd.map_or(-1, |fd| fd.as_raw_fd()))
                    ),
                    attr(IFLA_XDP_FLAGS as u16, value(flags)),
                    optional_attr(IFLA_XDP_EXPECTED_FD as u16, expected_fd),
                ]
            ),
        ]
    )?;
...
pub(crate) fn netlink_qdisc_add_clsact(if_index: i32) -> Result<(), NetlinkError> {
    ...
    send_netlink!(
        ...
        [
            ...
            attr(TCA_KIND as u16, bytes(c"clsact".to_bytes_with_nul())),
        ]
    )?;
    for msg in sock.recv() {
        msg?;
...
pub(crate) fn netlink_qdisc_attach(
    ...
    send_netlink!(
        ...
        [
            ...
            attr(TCA_KIND as u16, bytes(c"bpf".to_bytes_with_nul())),
            nested_attr(
                TCA_OPTIONS as u16,
                [
                    optional_attr(TCA_BPF_CLASSID as u16, classid.map(u32::from)),
                    attr(TCA_BPF_FD as u16, value(prog_fd.as_raw_fd())),
                    attr(TCA_BPF_NAME as u16, bytes(prog_name)),
                    attr(TCA_BPF_FLAGS as u16, value(TCA_BPF_FLAG_ACT_DIRECT)),
                ]
            ),
        ]
    )?;
    ...
    fn test_send_nested_attrs(#[case] classid: Option<u32>) {
        ...
        send_netlink!(
            ...
            [nested_attr(
                TCA_OPTIONS as u16,
                [
                    optional_attr(TCA_BPF_CLASSID as u16, classid),
                    attr(TCA_BPF_NAME as u16, bytes(c"ab".to_bytes_with_nul())),
                    attr(TCA_BPF_FLAGS as u16, value(TCA_BPF_FLAG_ACT_DIRECT)),
                ]
            ),]
        )
        ...
        // 16-byte message header, 4-byte nested header and up to three 8-byte
        // attributes. One extra byte makes an oversized message fail below.
        let mut buf = Vec::new();
        let len = receiver.take(45).read_to_end(&mut buf).unwrap();
        assert_eq!(len, if classid.is_some() { 44 } else { 36 });
        ...
        let mut iter = NlAttrsIterator::new(&msg.data);
