...
///
/// `time_enabled` and `time_running` can differ when the kernel multiplexes
/// the group. If `time_running` is nonzero, each counter can be normalized as
/// `value * time_enabled / time_running`. A zero `time_running` means that the
/// group was never scheduled.
#[derive(Clone, Copy, Debug, Eq, PartialEq)]
pub struct PerfEventGroupRead<'a> {
    /// Counter values in the same order as the configurations passed to
    /// [`PerfEventGroup::open`].
    pub values: &'a [u64],
    /// Time in nanoseconds for which the group was enabled.
    pub time_enabled: u64,
    /// Time in nanoseconds for which the group was running on the PMU.
...
impl PerfEventGroup {
    ...
    pub fn open(
        ...
        let mut group_members = Vec::with_capacity(members.size_hint().0);
        for config in members {
            let event_index = group_members.len() + 1;
            let event = perf_event_open_group_member(config, scope, Some(leader.as_fd())).map_err(
                |io_error| PerfEventGroupOpenError {
                    event_index,
                    io_error,
                },
            )?;
            group_members.push(event);
        }

        let read_buffer = vec![0; group_members.len() + 4].into_boxed_slice();
        Ok(Self {
            leader,
            members: group_members.into_boxed_slice(),
            read_buffer,
        })
    }
    ...
    ///
    /// This does not reset `time_enabled` or `time_running`.
    ///
    /// # Errors
    ///
    ...
    pub fn read(&mut self) -> io::Result<PerfEventGroupRead<'_>> {
        ...
        let [event_count, time_enabled, time_running, values @ ..] = self.read_buffer.as_ref()
        else {
            return Err(io::Error::new(
                io::ErrorKind::InvalidData,
                "perf event group returned an incomplete header",
            ));
        };
