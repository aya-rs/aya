...
///
/// `enabled` and `running` can differ when the kernel multiplexes the group.
/// If `running` is nonzero, each counter can be normalized as
/// `u128::from(value) * enabled.as_nanos() / running.as_nanos()`. A zero
/// `running` means that the group was never scheduled.
#[derive(Clone, Copy, Debug, Eq, PartialEq)]
pub struct PerfEventGroupRead<'a> {
    /// Counter values in the same order as the configurations passed to
    /// [`PerfEventGroup::open`].
    pub values: &'a [u64],
    /// Time for which the group was enabled.
    pub enabled: Duration,
    /// Time for which the group was running on the PMU.
...
impl PerfEventGroup {
    ...
    ///
    /// This does not reset [`PerfEventGroupRead::enabled`] or
    /// [`PerfEventGroupRead::running`].
    ///
    /// # Errors
    ///
    ...
    pub fn read(&mut self) -> io::Result<PerfEventGroupRead<'_>> {
        ...
        Ok(PerfEventGroupRead {
            values,
            enabled: Duration::from_nanos(*enabled_nanos),
            running: Duration::from_nanos(*running_nanos),
        })
    }
}
