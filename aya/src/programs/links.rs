    ...
    T::Id: hashbrown::Equivalent<T> + Eq + std::hash::Hash,
{
    ...
    /// Inserts a link after the kernel-side attach has already happened.
    ///
    /// Prefer [`Links::insert`] when the link id is known before attach, so link creation
    /// can be skipped on duplicates. If this returns [`ProgramError::AlreadyAttached`],
    /// the rejected link is dropped and may run its RAII detach path.
    pub(crate) fn insert_after_attach(&mut self, link: T) -> Result<T::Id, ProgramError> {
        let link_id = link.id();
        self.insert(link_id, || Ok(link))
    }
