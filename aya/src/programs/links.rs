...
pub struct LinkInfo(pub(crate) bpf_link_info);
...
impl LinkInfo {
    ...
    pub fn link_type(&self) -> Result<LinkType, LinkError> {
        bpf_link_type::try_from(self.0.type_)
            .map_err(|_| LinkError::UnexpectedLinkType)
            .and_then(|link_type| {
                LinkType::try_from(link_type).map_err(|_| LinkError::UnexpectedLinkType)
            })
    }
}
...
impl TryFrom<bpf_link_type> for LinkType {
    ...
    fn try_from(link_type: bpf_link_type) -> Result<Self, Self::Error> {
        ...
        match link_type {
            ...
            __MAX_BPF_LINK_TYPE => Err(LinkError::InvalidLink),
        }
    }
}
...
pub enum LinkError {
    ...
    /// The link is not of the expected type.
