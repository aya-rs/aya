...
impl SocketFilter {
    ...
    ///
    /// The socket passed to `attach` must remain open until the link is detached. If the
    /// link is taken with [`SocketFilter::take_link`], keep the socket open until the
    /// owned [`SocketFilterLink`] is detached or dropped.
