...
///
/// Both attachment types use kernel-managed socket slots. Regular filters are
/// scoped to one socket; reuseport selectors are scoped to the whole
/// `SO_REUSEPORT` group. Attaching a new program replaces the current program
/// in that slot, and detaching clears the slot regardless of which program
/// installed it. For that reason, [`SocketFilter`] does not expose a
/// link-style attachment handle or automatically track these attachments for
/// cleanup. Dropping [`SocketFilter`] or [`crate::Ebpf`] does not detach the
/// program; call [`SocketFilter::detach`] explicitly when you want to remove
/// it, or close the socket.
///
...
impl SocketFilter {
    ...
    ///
    /// [`SocketFilterAttachType::SocketFilter`] attaches the program as a regular
    /// socket filter. If the socket already has a filter attached, attaching
    /// again replaces the current filter instead of returning an
    ...
    /// The socket must already be configured with `SO_REUSEPORT`. The program
    /// return value is interpreted as the selected socket index in the
    /// reuseport group. `SO_ATTACH_REUSEPORT_EBPF` support for socket filter
    /// programs was added for UDP sockets in Linux 4.5 and TCP sockets in
    /// Linux 4.6.
    ///
    /// Regular socket filters and reuseport selectors both use one
    /// kernel-managed slot: [`SocketFilterAttachType::SocketFilter`] is
    /// socket-scoped, and [`SocketFilterAttachType::ReusePort`] is
    /// reuseport-group-scoped.
    /// Attaching a new program replaces the current program in that slot instead
    /// of returning an already-attached error. For reuseport groups, attaching
    /// through any socket in the group replaces the program used by the entire
    /// group. The kernel detach API also clears the current slot; it cannot
    /// detach a specific program attachment. For that reason, socket filters do
    /// not provide link-level RAII semantics. Dropping [`SocketFilter`] or
    ...
    /// `SO_DETACH_REUSEPORT_BPF` is available on Linux 5.3 and later.
    ///
