...
impl SocketFilter {
    ...
    ///
    /// The file descriptor passed to `attach` must remain open until the link is
    /// detached. The link stores that raw file descriptor for detach; closing it first
    /// lets the descriptor number be reused, so detach or drop may operate on an
    /// unrelated socket. If the link is taken with [`SocketFilter::take_link`], keep the
    /// file descriptor open until the owned [`SocketFilterLink`] is detached or dropped.
