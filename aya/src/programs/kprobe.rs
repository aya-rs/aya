...
impl KProbe {
        ...
        I: IntoIterator<Item = KProbeAttachPoint<'a>>,
    {
        ...
        // Consuming the caller's iterator can panic after some points are attached.
        // On kernels older than 4.17, closing perf fds alone leaves tracefs probe
        // events behind. Detach partial links on errors or unwinding until the
        // KProbeLink below takes ownership.
        let mut links = scopeguard::guard(
            ManyProbeLinks::from_first_link(first_link, capacity),
            ManyProbeLinks::detach,
        );
        ...
        for (index, point) in iter::once(second).chain(points).enumerate() {
            links
                ...
                .map_err(|error| attach_error(index + 1, &point, error))?;
        }

        data.links.insert(KProbeLink::from(ProbeLinkInner::Many(
            ScopeGuard::into_inner(links),
        )))
    }
