...
impl CgroupSysctl {
    ...
    pub fn load(&mut self) -> Result<(), ProgramError> {
        let Self { data } = self;
        load_program_with_attach_type(BPF_PROG_TYPE_CGROUP_SYSCTL, BPF_CGROUP_SYSCTL, data)
    }
    ...
    pub fn attach<T: AsFd>(
        ...
        if KernelVersion::at_least(5, 7, 0) {
            ...
            let link = CgroupSysctlLink::new(CgroupSysctlLinkInner::Fd(FdLink::new(link_fd)));
            let link_id = link.id();
            self.data.links.insert(link_id, || Ok(link))
        } else {
            let link = ProgAttachLink::attach(prog_fd, cgroup_fd, attach_type, mode)?;

            let link = CgroupSysctlLink::new(CgroupSysctlLinkInner::ProgAttach(link));
            let link_id = link.id();
            self.data.links.insert(link_id, || Ok(link))
        }
    }
