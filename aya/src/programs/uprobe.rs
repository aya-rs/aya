        ...
        T: AsRef<Path>,
    {
        ...
        for (index, offset) in offsets.into_iter().enumerate() {
            ...
            match link {
                ...
                Err(error) => {
                    let _cleanup_result: Result<(), ProgramError> = detach_links(links);
        ...
        if links.len() == 1 {
            return data.links.insert(UProbeLink::from(links.pop().unwrap()));
        }
...
fn detach_links(links: Vec<PerfLinkInner>) -> Result<(), ProgramError> {
