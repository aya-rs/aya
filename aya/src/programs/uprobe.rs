...
impl UProbe {
    ...
    pub fn attach<'a, T, Point, Points>(
        &mut self,
        points: Points,
        target: T,
        pid: Option<u32>,
    ...
    where
        T: AsRef<Path>,
        Point: Into<UProbeAttachPoint<'a>>,
        Points: IntoIterator<Item = Point>,
    {
        let proc_map = pid.map(ProcMap::new).transpose()?;
        let resolved_path = resolve_attach_path(target.as_ref(), proc_map.as_ref())?;
        let points: Vec<UProbeAttachPoint<'a>> = points.into_iter().map(Into::into).collect();
