...
use crate::{
    VerifierLogLevel,
    programs::{
        FdLink, Link as _, PerfLinkIdInner, PerfLinkInner, PerfLinkLeaf, ProgramData, ProgramError,
    ...
    I::Item: Into<UProbeAttachPoint<'a>>,
{
    ...
    // Defer symbol resolution until all points are collected so the object
    // file is mapped once and additional symbol offsets are applied in one pass.
    ...
    fn attach_single_impl(
        ...
        for (index, offset) in resolved.offsets().iter().copied().enumerate() {
            ...
            match link {
                ...
                Err(error) => {
                    let cleanup_error = PerfLinkInner::Many(links).detach().err().map(Box::new);
...
impl_try_from_fdlink!(
    ...
    link_types = [
        ...
        bpf_link_type::BPF_LINK_TYPE_UPROBE_MULTI,
    ]
);
