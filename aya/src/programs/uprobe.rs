...
{
    let points = once(first).chain(rest);
    ...
    if let [offset] = offsets.as_slice() {
        ...
        Ok(ResolvedPoints::One { offset, cookie })
    } else {
            ...
            ResolvedPoints::Many { offsets, cookies } => {
                ...
                for (index, offset) in offsets.iter().copied().enumerate() {
                    ...
                    match link {
                        ...
                        Err(error) => {
                            let cleanup_error =
                                PerfLinkInner::Many(links).detach().err().map(Box::new);
...
fn resolve_pending_symbols(
    ...
    let first_symbol = requests
        .first()
        .map(|(_, symbol, _)| *symbol)
        .unwrap_or_default();
    let symbol_error = |error: ResolveSymbolError| {
    ...
    // Index pending requests by symbol name so each object symbol table is
    // scanned once. The value stores indices into `requests` and
    // `resolved_offsets`, allowing duplicate symbol requests to share the same
    // resolved base offset before per-request additional offsets are applied.
    let symbol_requests = build_symbol_requests(requests);
    let mut resolved_offsets = vec![None; requests.len()];
    let data = MMap::map_copy_read_only(path)
        .map_err(|error| symbol_error(ResolveSymbolError::Io(error)))?;
    let obj = object::read::File::parse(data.as_ref())
        .map_err(|error| symbol_error(ResolveSymbolError::Object(error)))?;
    resolve_symbols_in_object(&obj, &symbol_requests, &mut resolved_offsets)
        .map_err(&symbol_error)?;

    let unresolved_first_symbol = requests
        .iter()
    ...
    if let Some(unresolved_first_symbol) = unresolved_first_symbol {
        ...
        let data = MMap::map_copy_read_only(&debug_path).map_err(|e| {
            symbol_error(ResolveSymbolError::DebuglinkAccessError(
                debug_path.clone().into_owned(),
                e,
            ))
        })?;
        let debug_obj = object::read::File::parse(data.as_ref())
            .map_err(|error| symbol_error(ResolveSymbolError::Object(error)))?;
        verify_build_ids(&obj, &debug_obj, unresolved_first_symbol).map_err(&symbol_error)?;
        resolve_symbols_in_object(&debug_obj, &symbol_requests, &mut resolved_offsets)
            .map_err(symbol_error)?;
    }

    for ((index, symbol, additional), offset) in requests.iter().zip(&resolved_offsets) {
...
mod tests {
    ...
    #[test]
    fn test_resolve_symbols_in_object_resolves_mixed_requests() {
        let mut bytes =
        ...
        // Model an original attach-point list shaped like:
        //   0: symbol "first" + 0x4
        //   1: absolute offset 0x88
        //   2: symbol "second"
        //   3: symbol "first" + 0x8
        //
        // Each request is `(attach point index, symbol, additional offset)`.
        // There is no request for index 1 because absolute offsets do not need
        // symbol resolution.
        let requests = [(0, "first", 0x4), (2, "second", 0), (3, "first", 0x8)];
        let symbol_requests = build_symbol_requests(&requests);
        let mut resolved_offsets = vec![None; requests.len()];

        resolve_symbols_in_object(&obj, &symbol_requests, &mut resolved_offsets).unwrap();
        assert_eq!(resolved_offsets, vec![Some(0x10), Some(0x24), Some(0x10)]);
    }

    #[test]
    fn test_uprobe_link_into_fd_links_single() {
        let fd = crate::MockableFd::mock_signed_fd();
        let link = UProbeLink::from(PerfLinkInner::from(mock_fd_link(fd)));
    ...
    fn test_uprobe_link_into_fd_links_many() {
        ...
        let link = UProbeLink::from(PerfLinkInner::Many(vec![
            PerfLinkLeaf::from(mock_fd_link(first_fd)),
            PerfLinkLeaf::from(mock_fd_link(second_fd)),
        ]));
