...
pub enum UProbeAttachLocationError {
    /// The instruction address is below the containing section address.
    #[error(
        "instruction address {instruction_address:#x} \
        is below section address {section_address:#x}"
    )]
    InstructionAddressBelowSection {
...
impl UProbeAttachLocation<'static> {
    ...
    ///
    /// The `instruction_address` must be within the section described by
    /// `section_address` and `section_offset`.
    ///
    /// # Errors
    ///
    /// Returns [`UProbeAttachLocationError::InstructionAddressBelowSection`] if
    /// `instruction_address` is below `section_address`.
    ///
    ...
    pub fn from_virtual_address(
        ...
        instruction_address
            ...
            .ok_or(UProbeAttachLocationError::InstructionAddressBelowSection {
                instruction_address,
                section_address,
            })?
            .checked_add(section_offset)
            .ok_or(UProbeAttachLocationError::FileOffsetOverflow {
                instruction_address,
                section_address,
                section_offset,
            })
            .map(Self::AbsoluteOffset)
    }
}
...
mod tests {
    ...
    fn test_uprobe_attach_location_from_virtual_address_errors_on_underflow() {
        assert_matches!(
            ...
            Err(UProbeAttachLocationError::InstructionAddressBelowSection {
