...
mod tests {
    ...
    fn test_uprobe_link_into_fd_links_single() {
        ...
        let link = UProbeLink::from(PerfLinkInner::from(FdLink::new(file.into())));

        let fd_links = link.into_fd_links().unwrap();
        assert_eq!(fd_links.len(), 1);
    }

    #[test]
    fn test_uprobe_link_into_fd_links_multi() {
        let first = fs::File::open("/dev/null").unwrap();
        let second = fs::File::open("/dev/null").unwrap();
        let link = UProbeLink::from(PerfLinkInner::Multi(vec![
            PerfLinkLeaf::from(FdLink::new(first.into())),
            PerfLinkLeaf::from(FdLink::new(second.into())),
        ]));

        let fd_links = link.into_fd_links().unwrap();
        assert_eq!(fd_links.len(), 2);
    }
