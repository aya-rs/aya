...
impl<'a> ProcMapEntry<'a> {
    fn parse(line: &'a [u8]) -> Result<Self, ProcMapError> {
        ...
        let path = parts.next().and_then(|path| {
            if path == b"[vdso]" {
                None
            } else {
...
mod tests {
    ...
    fn test_parse_proc_map_entry_shared_lib() {
        assert_matches!(
            ...
            Ok(ProcMapEntry {
                _address: 0x7ffd6fbea000,
                _address_end: 0x7ffd6fbec000,
                _perms,
                _offset: 0,
                _dev,
                _inode: 0,
                path: None,
            }) if _perms == "r-xp" && _dev == "00:00"
        );
    }
    ...
    fn test_parse_proc_map_entry_absolute_path() {
        assert_matches!(
            ...
            Ok(ProcMapEntry {
                _address: 0x7f1bca83a000,
                _address_end: 0x7f1bca83c000,
                _perms,
                _offset: 0x00036000,
                _dev,
                _inode: 2895508,
                path: Some(path),
            }) if _perms == "rw-p" && _dev == "fd:01" && path == Path::new("/usr/lib64/ld-linux-x86-64.so.2"
        ));
    }
    ...
    fn test_parse_proc_map_entry_all_zeros() {
        assert_matches!(
            ...
            Ok(ProcMapEntry {
                _address: 0x7f1bca5f9000,
                _address_end: 0x7f1bca601000,
                _perms,
                _offset: 0,
                _dev,
                _inode: 0,
                path: None,
            }) if _perms == "rw-p" && _dev == "00:00"
        );
    }
    ...
    fn test_proc_map_with_multiple_lib_entries() {
        let proc_map_libs = ProcMap {
            pid: 0xdead,
            data: br#"7f372868000-7f3722869000 r--p 00000000 00:24 18097875                   /usr/lib64/ld-linux-x86-64.so.2
