...
use crate::programs::{
    ExpectedAttachType, FdLink, FdLinkId, LsmAttachType, ProgramData, ProgramError, ProgramType,
...
impl Lsm {
    ...
    pub fn load(&mut self, lsm_hook_name: &str, btf: &Btf) -> Result<(), ProgramError> {
        self.data.expected_attach_type = Some(ExpectedAttachType::AttachType(BPF_LSM_MAC));
