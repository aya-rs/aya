...
pub(crate) fn is_syscall_wrapper_supported() -> bool {
    ...
    for syscall_name in SYS_BPF_SYMBOLS {
        let args = ProbeEventArgs {
            target: super::kprobe::KProbeAttachTarget {
                function: OsStr::new(syscall_name),
                offset: 0,
                pid: Some(process::id()),
            },
        ...
        let is_supported = if supports_probe {
            create_as_probe::<super::KProbe>(args).is_ok()
        } else {
            create_as_trace_point::<super::KProbe>(args).is_ok()
        };
        if is_supported {
            return true;
        }
