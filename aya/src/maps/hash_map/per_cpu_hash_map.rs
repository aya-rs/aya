...
impl<T: Borrow<MapData>, K: Pod, V: Pod> PerCpuHashMap<T, K, V> {
    ...
    pub fn batch_lookup_and_delete(
        ...
        in_batch: Option<&K>,
        flags: u64,
    ) -> Result<(Vec<K>, Vec<PerCpuValues<V>>, Option<K>), MapError> {
        let fd = self.inner.borrow().fd().as_fd();

        bpf_map_lookup_and_delete_batch_per_cpu(fd, in_batch, batch_size, flags).map_err(
...
mod tests {
    ...
    fn test_batch_lookup_and_delete_with_entries() {
        ...
        assert_matches!(result, Ok((keys, values, cursor)) => {
            ...
            for cpu in 0..nr_cpus {
                assert_eq!(values[0][cpu], 100 + cpu as u32);
            }

            // Check per-CPU values for key 20
            assert_eq!(values[1].len(), nr_cpus);
            for cpu in 0..nr_cpus {
                assert_eq!(values[1][cpu], 200 + cpu as u32);
            }
