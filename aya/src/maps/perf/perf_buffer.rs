...
impl PerfBuffer {
        ...
        F: FnMut(C, PerfEvent<'_>) -> ControlFlow<B, C>,
    {
        ...
        let mut guard = scopeguard::guard((initial_tail, false), |(tail, dirty)| {
            if dirty {
        ...
        let mut acc = init;
        while guard.0 != data_head {
            let event_start = (guard.0 % mmap_size as u64) as usize;
            debug_assert_eq!(
                event_start % 8,
                0,
            ...
            guard.0 = guard.0.wrapping_add(event_size as u64);
            guard.1 = true;

            let perf_event = match event_type {
                x if x == PERF_RECORD_SAMPLE as u32 => {
                    let mut size_buf = [0u8; size_of::<u32>()];
                    // SAFETY: `base`/`mmap_size` describe a valid mmap region;
                    // the kernel will not overwrite these bytes until
                    // `data_tail` is committed on scope exit.
                    let (head, tail) = unsafe {
                        slice_from_mmap(
                            (event_start + size_of::<perf_event_header>()) % mmap_size,
                            size_buf.len(),
                            base,
                            mmap_size,
                        )
                    };
                    size_buf[..head.len()].copy_from_slice(head);
                    size_buf[head.len()..].copy_from_slice(tail);
                    ...
                    let (head, tail) =
                        unsafe { slice_from_mmap(sample_start, sample_size, base, mmap_size) };
                    PerfEvent::Sample { head, tail }
                }
                x if x == PERF_RECORD_LOST as u32 => {
                    let mut count_buf = [0u8; size_of::<u64>()];
                    // `PERF_RECORD_LOST` layout is
                    // `{ header, u64 id, u64 lost, sample_id }` [1]; skip
                    // past `id` to read the `lost` count.
                    //
                    // [1]: https://github.com/torvalds/linux/blob/05f7e89a/include/uapi/linux/perf_event.h#L908-L914
                    //
                    // SAFETY: same as above.
                    let (head, tail) = unsafe {
                        slice_from_mmap(
                    ...
                    count_buf[..head.len()].copy_from_slice(head);
                    count_buf[head.len()..].copy_from_slice(tail);
                    PerfEvent::Lost {
...
/// Slices of `len` bytes from the perf ring buffer at `base`, starting at
/// `start` and possibly wrapping at `mmap_size`. The second slice is empty
/// when the range is contiguous; together they cover exactly `len` bytes.
///
/// # Safety
///
/// `base` must point to a valid mmap region of at least `mmap_size` bytes.
unsafe fn slice_from_mmap<'a>(
    start: usize,
    len: usize,
    base: *const u8,
    mmap_size: usize,
) -> (&'a [u8], &'a [u8]) {
    debug_assert!(start < mmap_size, "start={start} mmap_size={mmap_size}");
    debug_assert!(len <= mmap_size, "len={len} mmap_size={mmap_size}");
    if let Some(second) = (start + len).checked_sub(mmap_size) {
        let first = mmap_size - start;
        // SAFETY: `[start, mmap_size)` and `[0, second)` are disjoint ranges
        // within the mmap region.
        let head = unsafe { slice::from_raw_parts(base.add(start), first) };
        let tail = unsafe { slice::from_raw_parts(base, second) };
        (head, tail)
    } else {
        // SAFETY: `start + len <= mmap_size`, contiguous within mmap.
        let head = unsafe { slice::from_raw_parts(base.add(start), len) };
        (head, &[])
    }
}
