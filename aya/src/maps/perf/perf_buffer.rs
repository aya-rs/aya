...
/// Copy `out_buf.len()` bytes starting at `start` from the perf ring buffer
/// at `base`, handling wrap-around at `mmap_size`.
///
/// `start` must be less than `mmap_size`; `out_buf.len()` must not exceed
/// `mmap_size`.
///
/// # Safety
///
/// `base` must point to a valid mmap region of at least `mmap_size` bytes.
fn fill_from_mmap(start: usize, base: *const u8, mmap_size: usize, out_buf: &mut [u8]) {
    debug_assert!(start < mmap_size);
    let len = out_buf.len();
    debug_assert!(len <= mmap_size);

    if start + len <= mmap_size {
        // SAFETY: `start + len <= mmap_size`, so the read is entirely within
        // the mmap region.
        unsafe {
        ...
        // SAFETY: `[start, mmap_size)` and `[0, len - first)` are disjoint
        // ranges within the mmap region; together they cover the wrapped read.
        unsafe {
            out_buf[..first].copy_from_slice(slice::from_raw_parts(base.add(start), first));
            out_buf[first..].copy_from_slice(slice::from_raw_parts(base, len - first));
        }
    }
}
