...
impl CgroupStorageKey {
    ...
    const fn to_kernel(self) -> bpf_cgroup_storage_key {
        ...
        // Zero the kernel struct so its tail padding is initialized before the
        // key crosses the syscall boundary.
        let mut key = unsafe { mem::zeroed::<bpf_cgroup_storage_key>() };
        key.cgroup_inode_id = cgroup_inode_id;
