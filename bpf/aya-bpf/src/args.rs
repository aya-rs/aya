...
impl<T> FromPtRegs for *const T {
    ...
    fn from_ip(ctx: &pt_regs) -> Option<Self> {
        unsafe { bpf_probe_read_kernel(&ctx.rip).map(|v| v as *const _).ok() }
    }
}
...
impl<T> FromPtRegs for *const T {
    ...
    fn from_ip(ctx: &pt_regs) -> Option<Self> {
        unsafe {
            bpf_probe_read_kernel(&ctx.uregs[12])
                .map(|v| v as *const _)
                .ok()
        }
    }
...
impl<T> FromPtRegs for *const T {
    ...
    fn from_ip(ctx: &pt_regs) -> Option<Self> {
        unsafe { bpf_probe_read_kernel(&ctx.pc).map(|v| v as *const _).ok() }
    }
}
...
impl<T> FromPtRegs for *mut T {
    ...
    fn from_ip(ctx: &pt_regs) -> Option<Self> {
        unsafe { bpf_probe_read_kernel(&ctx.rip).map(|v| v as *mut _).ok() }
    }
}
...
impl<T> FromPtRegs for *mut T {
    ...
    fn from_ip(ctx: &pt_regs) -> Option<Self> {
        unsafe { bpf_probe_read_kernel(&ctx.pc).map(|v| v as *mut _).ok() }
    }
}
