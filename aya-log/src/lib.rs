...
impl EbpfLogger<&'static dyn Log> {
    ...
    ///
    /// Attaches to the logs produced by `program_id`. Can be used to read logs
    /// generated by a pinned program. The log records will be written to the
    /// default logger. See [`log::logger`].
...
impl<T> Formatter<*const T> for PointerFormatter {
    #[expect(clippy::pointer_format, reason = "that's the point")]
