...
pub trait StackIdContext: EbpfContext + sealed::StackIdContext {
    ...
    /// # Safety
    ///
    /// The caller must ensure the context pointer wrapped by `self` is the
    /// one passed by the kernel verifier for the current program invocation.
    /// Sealing this trait guarantees the verifier accepts the helper call,
    /// but not that `self.as_ptr()` is a live BPF context pointer; that
    /// invariant must be upheld at context construction time.
    unsafe fn get_stackid<M: StackTraceMap>(&self, map: &M, flags: u64) -> Result<c_long, i32> {
        let ret =
