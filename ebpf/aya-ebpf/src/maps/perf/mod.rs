...
pub struct PerfEventValue {
    ...
    pub counter: u64,
    /// Time in nanoseconds for which the event was enabled.
    pub time_enabled: u64,
    /// Time in nanoseconds for which the event was running.
...
pub(crate) fn read_value(map: *mut c_void, index: u32) -> Result<PerfEventValue, i32> {
    ...
    Ok(PerfEventValue {
        ...
        time_enabled: value.enabled,
        time_running: value.running,
    })
}
