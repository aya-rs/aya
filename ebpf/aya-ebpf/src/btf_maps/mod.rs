...
macro_rules! btf_map_def {
    (
        $(#[$attr:meta])*
        $vis:vis struct $name:ident<T $(, const $cg_name:ident : usize $(= $cg_default:tt)?)*>,
        ...
        #[repr(C)]
        #[allow(dead_code)]
        $vis struct $name<T $(, const $cg_name : usize $(= $cg_default)?)*> {
        ...
        unsafe impl<T: Sync $(, const $cg_name : usize)*> Sync for $name<T $(, $cg_name)*> {}

        impl<T $(, const $cg_name : usize)*> Default for $name<T $(, $cg_name)*> {
            fn default() -> Self {
                Self::new()
            }
        }

        impl<T $(, const $cg_name : usize)*> $name<T $(, $cg_name)*> {
