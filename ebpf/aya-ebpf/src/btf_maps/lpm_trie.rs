...
impl<K, V, const MAX_ENTRIES: usize, const FLAGS: usize> LpmTrie<K, V, MAX_ENTRIES, FLAGS> {
    // Compile-time enforcement of the kernel constraints in
    // kernel/bpf/lpm_trie.c and of the alignment invariant on the returned
    // reference. Referenced from every inherent method so any invalid
